...
class SpeakerActionSignalDistribution():
    ...
    def __call__(self, trueGoal):
        ...
        dIYUtility = self.getActionUtility2(targetAction, trueGoal, self.signalerLocation)
        ...
        #signalDF = pd.DataFrame(signalDF2[0]*speakerSignalPDF['s1(msg|w)'])

        #print(pd.DataFrame(signalDF.values*speakerSignalPDF.values, columns=signalDF.columns, index=signalDF.index))
    ...
    def getExpectation(self, x, trueGoal, receiver = False):
        ...
        #getReceiverUtility = lambda x: self.getActionUtility(getIntentionLocation(x.index.get_level_values(NC.INTENTIONS)[0]), trueGoal, self.receiverLocation, x, True)*x[x.columns[0]]
        def getReceiverUtility(x):
            #print(x)
            
            #print(x[x.columns[0]])
            return (self.getActionUtility(self.getIntentionLocation(x.index.get_level_values(NC.INTENTIONS)[0]), trueGoal, self.receiverLocation, x, True)*x[x.columns[0]])
        
        utilities = receiverPDF.groupby(receiverPDF.index.names).apply(getReceiverUtility)
        #print(utilities)
        return(sum(utilities))
    
    def getIntentionLocation(self, intention):
        return [loc for loc, item in self.targetDictionary.items() if item == intention][0]
    
   
    

    def getActionUtility(self, action, goal, startingPoint, x = None, receiver = False):
        utility = self.getCost(startingPoint, action)
        if self.targetDictionary[action] == goal:
            utility += self.reward
        #print(x.index.get_level_values(NC.INTENTIONS)[0])
        if not receiver:
            utility -= self.getSignalCost(x.index.get_level_values(NC.INTENTIONS)[0] ) 
        return(utility)

    def getActionUtility2(self, action, goal, startingPoint):
        utility = self.getCost(startingPoint, action)
        if self.targetDictionary[action] == goal:
            utility += self.reward
        return(utility)
