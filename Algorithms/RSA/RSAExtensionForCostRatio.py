...
class SpeakerActionSignalDistribution_NoReceiverCost():
    def __init__(self, getPragmaticSpeaker, getPragmaticReceiver, targetDictionary, signalerLocation, receiverLocation, rationality, valueOfReward, getActionCost, receiverCostRatio, signalerInactionPossible = False):
    ...
    def getActionUtility(self, action, goal, startingPoint, receiver=False):
        utility = 0.0
        if not receiver:
            utility = self.getCost(startingPoint, action)*(1 - self.receiverCostRatio)
        else:
            utility = self.getCost(startingPoint, action)*self.receiverCostRatio
        if self.targetDictionary[action] == goal:
            utility += self.reward
        return(utility)

class SetupExperiment_RSASpeakerWithActionChoice_NoReceiverCost():
    def __init__(self, rationality, valueOfReward, receiverCostRatio, getActionCost=calculateLocationCost_TaxicabMetric, getSignalingCost = lambda x: 0, lexicon = consistencyLexiconRSA_Experiment,signalerInactionPossible= False ):
        self.rationality = rationality
    ...
    def __call__(self, targetPrior, signalSpace, targetDictionary, signalerLocation, receiverLocation, maxLayer = 1,getAllLayers = False):
        ...
        while currentLayer < maxLayer:
            ...
            getActingSpeaker = SpeakerActionSignalDistribution_NoReceiverCost(getPragmaticSpeaker=getSpeaker, 
                                                    ...
                                                    getActionCost=self.getActionCost, receiverCostRatio = self.receiverCostRatio, signalerInactionPossible = self.signalerInactionPossible )
            currentLayer += 1
