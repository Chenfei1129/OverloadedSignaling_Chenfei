...
def main():
	randomSeed = 987
	filename = './simulation_recursion_obstacleHNearRec_SQuit_a4_r8_seed987_6ItemFullVocab_300Runs.pkl'
	...
	barrierHNearRec = [((3, 5), (0, 1)),
					 ...
					 ((7, 6), (0, -1)),
					 ((8, 6), (0, -1))]
	policiesBarrierHNR = getPolicyDict(barrierHNearRec)
	getTransitionTable = createTransitionTable(gridDims[0], gridDims[1], actionSet)
	transitionTableBarrierHNR = getTransitionTable(barrierHNearRec)

	getCostObstacleHNR = SetupIndividualActionCost(policiesBarrierHNR, transitionTableBarrierHNR, 1)
	...
	# Modeling inference setup
	params_a4r8 = {'rationality' : alpha, 'reward': rewardValue, 'costFunction' : getCostObstacleHNR,'signalerInaction': signalerCanQuit}
	setupModInference = SetupModels(modelParameters=params_a4r8, modelNames = modNames)
	getSAUtil_BarrierHNR = SingleAgentUtility_CustomCostFunction(reward = rewardValue, getCostFunction = getCostObstacleHNR)

	#Simulation step
	getModelMetrics = SimulateModelFromDF(buildModels=setupModInference, getUtility=getSAUtil_BarrierHNR, signalerCanQuit=signalerCanQuit)
