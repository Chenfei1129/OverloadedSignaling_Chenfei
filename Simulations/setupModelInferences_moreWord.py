...
from Environments.Experiment.experimentConstruction import JointActionUtility, calculateLocationCost_TaxicabMetric #_CostRatioReceiver
from Environments.Experiment.costSignal import CalculateSignalCost 
from Environments.Experiment.setupInference_Experiment import SetupExperiment_SignalsSeparated
from Algorithms.RSA.SetupRSAInference_moreWord import SetupExperiment_RSAListenerInference, SetupExperiment_RSASpeakerWithActionChoice, SetupExperiment_RSAListenerInference, SetupExperiment_RSASpeakerInference
...
class SetupModels():
    ...
    def __call__(self, environmentParameters):
        ...
        #print(modelInferenceDictionary)
        return(modelInferenceDictionary)
        
    def setupIWInference(self, sampledEnvironmentParameters):
        runIWInference = SetupExperiment_SignalsSeparated(beta = self.modelParameters['rationality'],valueOfReward = self.modelParameters['reward'], getUtility = JointActionUtility, getCost = calculateLocationCost_TaxicabMetric, 
                                                         signalerInactionPossible=self.signalerInactionPossible,signalCost = CalculateSignalCost(self.signalCost) ) ########
    ...
    def setupRSAListeners(self, sampledEnvironmentParameters, maxReceiver):
        ...
        runRSAListenerInference = SetupExperiment_RSAListenerInference(self.modelParameters['rationality'],getCost = CalculateSignalCost(self.signalCost) )
    ...
    def setupRSASpeakers(self, sampledEnvironmentParameters, maxActingSpeakerLevel, maxLanguageOnlySpeakerLevel):
        ...
        if maxActingSpeakerLevel is not None:
            runRSASpeakerInference = SetupExperiment_RSASpeakerWithActionChoice(
                                                rationality = self.modelParameters['rationality'], 
                                                valueOfReward = self.modelParameters['reward'],
                                                getActionCost = calculateLocationCost_TaxicabMetric,
                                                getSignalingCost = CalculateSignalCost(self.signalCost), ###
        ...
        if maxLanguageOnlySpeakerLevel is not None:
            runRSASpeakerInference = SetupExperiment_RSASpeakerInference(self.rationality,CalculateSignalCost(self.signalCost) )
