...
from Environments.Experiment.experimentConstruction import JointActionUtility_CostlessReceiver
...
from Algorithms.RSA.RSAExtensionForCostlessReceiver import SpeakerActionSignalDistribution_NoReceiverCost, SetupExperiment_RSASpeakerWithActionChoice_NoReceiverCost
from Algorithms.JointUtility.utilityOnlyModel import UtilityDrivenSignaler_NoReceiverCosts, UtilityDrivenReceiver
...
class SetupModels_CostlessReceiver():
    ...
    def setupIWInference(self, sampledEnvironmentParameters):
        runIWInference = SetupExperiment_SignalsSeparated(self.modelParameters['rationality'], self.modelParameters['reward'], JointActionUtility_CostlessReceiver) ########
    ...
    def setupRSASpeakers(self, sampledEnvironmentParameters, maxActingSpeakerLevel, maxLanguageOnlySpeakerLevel):
        ...
        if maxActingSpeakerLevel is not None:
            runRSASpeakerInference = SetupExperiment_RSASpeakerWithActionChoice_NoReceiverCost(self.modelParameters['rationality'], self.modelParameters['reward']) #########3
    ...
    def setupJointUtilityInference(self, sampledEnvironmentParameters):
        ...
        signaler_JointUtility = UtilityDrivenSignaler_NoReceiverCosts(signalSpace=sampledEnvironmentParameters['signals'], ##############
                                          signalerLocation=sampledEnvironmentParameters['signalerLocation'], 
                                          receiverLocation=sampledEnvironmentParameters['receiverLocation'], 
                                          targetDictionary=sampledEnvironmentParameters['targetDictionary'], 
                                          valueOfReward=self.modelParameters['reward'], 
