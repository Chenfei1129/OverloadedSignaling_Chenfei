...
class SampleExperimentEnvironment2():
    ...
    def getFeaturesOfSampledTargets(self, targets):
        ...
        shapes = [feature for feature in ['circle', 'triangle', 'square' , 'oval'] if feature in featureList]
        sizes = [feature for feature in ['small', 'medium', 'large' , 'smallest'] if feature in featureList]
        for color in colors :
            for shape in shapes:
                for size in sizes:
                    featureList.append(color + ' ' + shape+ ' ' + size )
...
class SampleExperimentEnvironment3():
    def __init__(self, fixedEnvironmentParameters, fluidParameterSpaces):
        self.fixedEnvironmentParameters = fixedEnvironmentParameters
        self.parameterSpaces = fluidParameterSpaces
    def __call__(self, numItem, numberOfSignals, vocabProportion=False):
    ...
    def getFeaturesOfSampledTargets(self, targets):
        ...
        featureList = list(set(flatten(allFeatures)))
        #put in a dictionary. {feature name: possible} loop through the disctionary. {feature name: thing I can talk about in the environment} itertools package combination. recursive function
        colors = [feature for feature in ['green', 'purple', 'red', 'blue', 'orange'] if feature in featureList]
        shapes = [feature for feature in ['circle', 'triangle', 'square', 'oval', 'star'] if feature in featureList]
        sizes = [feature for feature in ['small', 'medium', 'large', 'smallest', 'largest' ] if feature in featureList]
        shades = [feature for feature in ['noshade',  'halfshade' , 'shade', 'quartershade', 'greyshade'] if feature in featureList]
        for color in colors :
            for shape in shapes:
                for size in sizes:
                    for shade in shades:
        ...
        for shape in shapes:
            for size in sizes:
                for shade in shades:
                    featureList.append(shape + ' ' + size + ' ' + shade )
...
class SampleExperimentEnvironment4():
    def __init__(self, fixedEnvironmentParameters, fluidParameterSpaces):
        self.fixedEnvironmentParameters = fixedEnvironmentParameters
        self.parameterSpaces = fluidParameterSpaces
    def __call__(self, numItem, numberOfSignals, vocabProportion=False):
    ...
    def getFeaturesOfSampledTargets(self, targets):
        ...
        sizes = [feature for feature in ['small', 'medium', 'large', 'smallest', 'largest' ] if feature in featureList]
        shades = [feature for feature in ['noshade',  'halfshade' , 'shade', 'quartershade', 'greyshade'] if feature in featureList]
        heights = [feature for feature in ['h1', 'h2', 'h3', 'h4', 'h5'] if feature in featureList]
        for color in colors :
            for shape in shapes:
                for size in sizes:
                    for shade in shades:
                        for height in heights:
                            featureList.append(color + ' ' + shape+ ' ' + size + ' ' + shade + ' ' + height)
        ...
        for color in colors :
            for shape in shapes:
                for size in sizes:
                    for height in heights:
                        featureList.append(color + ' ' + shape+ ' ' + size + ' ' + height)
        for color in colors :
            for shape in shapes:
                for shade in shades:
                    for height in heights:
                        featureList.append(color + ' ' + shape+ ' ' + shade + ' ' + height)                        
        for color in colors :
            for size in sizes:
                for shade in shades:
                    for height in heights:
                        featureList.append(color + ' ' + size + ' ' + shade + ' ' + height)                    
        for shape in shapes:
            for size in sizes:
                for shade in shades:
                    for height in heights:
                        featureList.append(shape+ ' ' + size + ' ' + shade + ' ' + height)
                        
        for color in colors :
            for shape in shapes:
                for height in heights:
                    featureList.append(color + ' ' + shape+ ' ' + height )
        for color in colors :
            for shape in shapes:
                for height in heights:
                    featureList.append(color + ' ' + shape+ ' ' + height)
        for color in colors :
            for size in shapes:
                for height in heights:
                    featureList.append(color + ' ' + size+ ' ' + height)           
        for shape in shapes:
            for size in sizes:
                for height in heights:
                    featureList.append(shape + ' ' + size + ' ' + height )
        for shape in shapes :
            for shade in shades :
                for height in heights:
                    featureList.append(shape + ' ' + shade + ' ' + height)                
        for size in sizes :
            for shade in shades :
                for height in heights:
                    featureList.append(size + ' ' + shade+ ' ' + height)
        ...
        for color in colors :
            for height in heights:
                featureList.append(color + ' ' + height)
        for shape in shapes :
            for height in heights:
                featureList.append(shape + ' ' + height)
        for size in sizes :
            for height in heights:
                featureList.append(size + ' ' + height)
        for shade in shades:
            for height in heights:
                featureList.append(shade + ' ' + height)
...
class SampleExperimentEnvironment_3():
    def __init__(self, fixedEnvironmentParameters, fluidParameterSpaces):
        self.fixedEnvironmentParameters = fixedEnvironmentParameters
        self.parameterSpaces = fluidParameterSpaces
    def __call__(self, numItem, numberOfSignals, vocabProportion=False):
